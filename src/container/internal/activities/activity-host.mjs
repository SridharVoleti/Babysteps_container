...
export function resolveApprovedServices(requiredServices = [], approvedServices = {}) {
  ...
  return Object.freeze(Object.fromEntries(requiredServices.map((name) => [name, approvedServices[name]])));
}

// Shared event-wiring used by both the standalone AH-001 host below and the
// AH-002 lifecycle manager, which wraps this channel with active-instance guarding.
export function createActivityEventChannel({ runtime, lifecycle, completion, onMeaningfulProgress = () => {}, onFailure = () => {}, emit = () => {} }) {
...
export function mountActivity({
  ...
  const events = createActivityEventChannel({ runtime, lifecycle, completion, onMeaningfulProgress, onFailure, emit });
